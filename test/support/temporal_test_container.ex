defmodule Temporal.TestContainer do
  ...
  defp start_new_container do
    ...
    case ensure_testcontainers_started() do
      :ok ->
        ...
        postgres_config =
          Testcontainers.Container.new("postgres:13")
        ...
        case Testcontainers.start_container(postgres_config) do
          {:ok, postgres_container} ->
            postgres_port =
              postgres_container.exposed_ports
            ...
            case wait_for_postgres_ready(postgres_port) do
              :ok ->
                ...
                temporal_config =
                  Testcontainers.Container.new(@temporal_image)
                  |> Testcontainers.Container.with_exposed_ports([
                    @temporal_grpc_port,
                    @temporal_ui_port
                  ])
      ...
      {:ok, socket} ->
        ...
        task =
          Task.async(fn ->
            ...
            wait_for_service_ready(grpc_port, 60)
          end)

        # Match increased retry timeout
        case Task.await(task, 65_000) do
          :ok ->
  ...
  def stop_container(container) do
    ...
    case get_container() do
      %{container: stored_container, postgres_container: postgres_container}
      when stored_container.container_id == container.container_id ->
        ...
        postgres_result =
          case Testcontainers.stop_container(postgres_container.container_id, Testcontainers) do
            :ok ->
