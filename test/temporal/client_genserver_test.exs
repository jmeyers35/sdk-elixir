defmodule Temporal.ClientGenServerTest do
  ...
  describe "configuration management" do
    test "uses application environment configuration" do
      # App env no longer affects defaults; verify opts-only behavior by passing none and using defaults
      original_config = Application.get_env(:temporal, Client)

      Application.put_env(:temporal, Client,
        host: "app-config:7233",
      ...
      assert state.config["target_host"] == "localhost:7233"
    ...
    test "command line options override application config" do
      # App env ignored; verify opts set namespace only
      original_config = Application.get_env(:temporal, Client)
      ...
      {:ok, client} = Client.start_link(namespace: "override-namespace")

      state = :sys.get_state(client)
      assert state.config["target_host"] == "localhost:7233"
      assert state.config["namespace"] == "override-namespace"
