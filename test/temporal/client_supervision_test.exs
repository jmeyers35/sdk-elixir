defmodule Temporal.ClientSupervisionTest do
  ...
  describe "supervisor integration" do
    ...
    test "supervisor restart strategies" do
      ...
      # Unchanged
      assert Process.whereis(:first_client) == first_pid
      # Restarted
      assert Process.whereis(:second_client) != second_pid
      # Also restarted
      assert Process.whereis(:third_client) != third_pid
  ...
  describe "application supervision tree" do
    test "client can be added to application supervision tree" do
      ...
      assert Enum.any?(children, fn {id, _, type, _} ->
               id == Temporal.TaskSupervisor and type == :supervisor
             end)
